...
class IEC104Wrapper():
    ...
    def create_apdu(self, frame, asdu_type, cot, originator_address, common_address, message, ssn = 0, rsn = 0):
        """
        Creates a IEC104 APDU without header based on the given parameters.
        :param frame: Frame format to be used.
        :param asdu_type: Type of the message as string.
        :param cot: A tuple containing the cause of transmission as string(e.g. periodic), P/N Bit(defaults to 0) and Testbit(defaults to 0).
        ...
        if type(apci) is str:
            return apci
        asdu = self.wrap_asdu(asdu_type, cot, originator_address, common_address, message)
        if type(asdu) is str:
            return asdu
        return apci + asdu
    ...
    def wrap_asdu(self, asdu_type, cot, originator_address, common_address, message):
        """
        Adds a IEC 104 ASDU to a struct based on the given parameters.
        :param asdu_type: Type of the message as string.
        :param cot: A tuple containing the cause of transmission as string(e.g. periodic), P/N Bit(defaults to 0) and Testbit(defaults to 0).
        :param originator_address: Originator address as integer.
        :param common_address: Common address of ASDUs as integer.
        ...
        if type(type_id) is str:
            return type_id
        cot = self.wrap_cause_of_transmission(cot, originator_address)
        if type(cot) is str:
            return cot
        ca = self.wrap_common_address(common_address)
        if type(ca) is str:
            return ca
        return type_id + cot + ca

    def wrap_asdu_type(self, asdu_type):
        """
        Adds a IEC 104 type identification to a struct based on the given ASDU type.
        :param asdu_type: Type of the message as string.
        :return: Struct containing an IEC 104 type identification as bytestring. ERROR if failed.
        """
        ...
        return struct.pack('<B', type_id) + vsq
    ...
    def wrap_cause_of_transmission(self, cot, originator_address):
        """
        ...
        pn = 0
        test = 0
        if not type(cot[0]) is str:
            return "ERROR: Cause of transmission has to be a string."
        if ("periodic" in cot[0]) or ("cyclic" in cot[0]):
            cause = 1
        if "spontaneous" in cot[0]:
            cause = 3
        if ("request" in cot[0]) or ("requested" in cot[0]):
            cause = 5
        if "activation" in cot[0]:
            cause = 6
        if "activation confirmation" in cot[0]:
            cause = 7
        if ("return information" in cot[0]) and ("remote command" in cot[0]):
            cause = 11
    ...
    def wrap_common_address(self, common_address):
        """
        Adds a IEC 104 common address to a struct based on the given parameters.
        :param common_address: Common address of ASDUs as integer.
        :param message: Message to be wrapped.
...
class TestWrapper(unittest.TestCase):
    ...
    def test_asdu_type(self):
        wrapper = IEC104Wrapper()
        self.assertEqual(b'\x07\x01', wrapper.wrap_asdu_type("M_BO_NA_1"))
        self.assertEqual(b'\x0d\x01', wrapper.wrap_asdu_type("M_ME_NC_1"))
        self.assertEqual(b'\x2d\x01', wrapper.wrap_asdu_type("C_SC_NA_1"))
        self.assertEqual(b'\x64\x01', wrapper.wrap_asdu_type("C_IC_NA_1"))
        self.assertEqual(b'\x66\x01', wrapper.wrap_asdu_type("C_RD_NA_1"))
    ...
    def test_asdu(self):
        wrapper = IEC104Wrapper()
        self.assertEqual(b'\x07\x01\x01\x00\x01\x00', wrapper.wrap_asdu("M_BO_NA_1", ("periodic", 0, 0), 0, 1, "test"))
        self.assertEqual("ERROR: The ASDU type was not recognized.", wrapper.wrap_asdu(1, ("periodic", 0, 0), 0, 1, "test"))
        self.assertEqual("ERROR: No cause of transmission was found.", wrapper.wrap_asdu("M_BO_NA_1", ("test", 0, 0), 0, 1, "test"))
        self.assertEqual("ERROR: Common address has to be an integer between 1 and 65535.", wrapper.wrap_asdu("M_BO_NA_1", ("periodic", 0, 0), 0, -1, "test"))

    def test_create_apdu(self):
        wrapper = IEC104Wrapper()
        self.assertEqual(b'\x02\x00\x02\x00\x07\x01\x01\x00\x01\x00', wrapper.create_apdu("i-frame", "M_BO_NA_1", ("periodic", 0, 0), 0, 1, "test", 1, 1))
        self.assertEqual("ERROR: No valid frame format was given.", wrapper.create_apdu("test", "M_BO_NA_1", ("periodic", 0, 0), 0, 1, "test", 1, 1))
        self.assertEqual("ERROR: The ASDU type was not recognized.", wrapper.create_apdu("i-frame", "test", ("periodic", 0, 0), 0, 1, "test", 1, 1))

    def test_create_header(self):
        wrapper = IEC104Wrapper()
        self.assertEqual(b'\x68\x0a', wrapper.create_apdu_header(b'\x02\x00\x02\x00\x07\x01\x01\x00\x01\x00'))
